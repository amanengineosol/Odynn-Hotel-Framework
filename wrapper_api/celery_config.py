...
CELERY_BROKER_URL = os.getenv(
    "CELERY_BROKER_URL",
    "amqp://admin:Pass123@rabbitmq:5672"
)

# DO NOT define CELERY_RESULT_BACKEND at all
...
# Force-disable result backend at runtime (bulletproof)
celery_app.conf.update(
    result_backend=None,
    task_ignore_result=True,
)

# A single fixed exchange named "Hotel" with queues under it
hotel_exchange = Exchange("Hotel", type="direct")

celery_app.conf.task_queues = [
    Queue("Hyatt", hotel_exchange, routing_key="Hyatt"),
    Queue("Marriott", hotel_exchange, routing_key="Marriott"),
    Queue("celery"),  # fallback/default queue
]

# Optional: dynamic queue setup
def setup_dynamic_queues(crawler_data):
    domain_exchange = Exchange("Hotel", type="direct")
