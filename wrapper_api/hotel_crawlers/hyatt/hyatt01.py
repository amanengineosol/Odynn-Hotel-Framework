...
stealth_js = """
    ...
    // Pass iframe/child frame checks
    window.top = window.self;
    
    // Mock permissions query to prevent detection
    ...
    // Patch to spoof WebGL Vendor and Renderer
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                if response.status_code == 200 and data_json and "roomRates" in decodedResponse and "lowestAvgPointValue" in decodedResponse:
                    ...
                    logger.error(f"Hyatt internal API failed with status {response.status_code}")
                    message = {
                        "details": f"Hyatt internal API failed with status {response.status_code}"
                    }
                    return self.build_response(success=False, data=message, status_code=response.status_code)
