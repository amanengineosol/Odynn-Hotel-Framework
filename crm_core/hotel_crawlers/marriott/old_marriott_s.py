...
class ExtractMarriott:
    ...
    async def get_search_data(self, hotel_id, check_in_date, check_out_date, guest_count, max_retries=3):
        ...
        async def capture_marriott_search(tab, event):
            ...
            # Only capture the Marriott rates API response
            if '/default.mi' in url or 'search/submitSearch.mi' in url or 'search/findHotels.mi' in url:
                try:
                    body = await tab.get_network_response_body(request_id)
        ...
        try:
            ...
            async with Chrome(options=options) as browser:
                ...
                try:
                    ...
                    await tab.go_to('https://www.marriott.com/')
                    human_delay(6, 12)
                    ...
                    find_hotels_btn = await tab.query("//button[normalize-space(text())='Find Hotels']")
