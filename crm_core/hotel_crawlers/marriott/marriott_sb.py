...
DEFAULT_CHECK_IN_DATE = '2025-12-11'
DEFAULT_CHECK_OUT_DATE = '2025-12-14'
...
class MarriottScraper:
    ...
    def _safe_human_type(self, selector: str, text: str, description: str, sleep_time: float = 1.0, word_delay: float = 0.3):
        """Wrapper for sb.type that types one word at a time with optional pauses."""
        try:
            ...
            for i, word in enumerate(words):
                self.sb.type(selector, word + (" " if i < len(words) - 1 else ""), by="css selector", timeout=10)
                self.sb.sleep(word_delay)
    ...
    def _navigate_and_search(self):
        ...
        self.sb.sleep(1)
        if not self._safe_human_type('input[id="downshift-1-input"]', self.location, "Destination Text",word_delay=0.4): return False
        self.sb.sleep(3)
        ...
        # selector = 'input[name="input-text-1 night"]'
        # date_range_value = "Tue, Nov 4 - Sat, Nov 8"
        #
        # self.sb.execute_script(
        #     f"document.querySelector('{selector}').value = '{date_range_value}';"
        # )
        ...
        selector = 'button.update-search-btn'
        ...
        self.sb.sleep(1)
        # XPath locator
        view_rates_xpath = "//a[contains(@class, 'view-rates-button-container') and @aria-label='View Rates']"
        ...
        print("✅ Clicked 'View Rates' button successfully!")
...
if __name__ == '__main__':
    scraper = MarriottScraper().get_search_data(
        check_in_date=DEFAULT_CHECK_IN_DATE,
        check_out_date=DEFAULT_CHECK_OUT_DATE,
        hotel_id="burah-sheraton-agoura-hills-hotel",
        guest_count = 1
    )
    print("\n--- FINAL CLIENT RESPONSE ---")
    print(json.dumps(scraper, indent=4))