from seleniumbase import SB

with SB(uc=True, test=True, locale="en", ad_block=True) as sb:
    url = "https://www.hyatt.com/"
    sb.activate_cdp_mode(url)
    sb.sleep(3.5)
    sb.click_if_visible('button[aria-label="Close"]')
    sb.click_if_visible("#onetrust-reject-all-handler")
    sb.sleep(1)
    location = "Hyatt Place Montreal - Downtown"
    sb.type('input[id="search-term"]', location)
    sb.sleep(1)
    sb.click('li[data-js="suggestion"]')
    sb.sleep(1)
    check_in_date = "2026-01-28",
    check_out_date = "2026-01-29",
    sb.execute_script(f"""
                    document.querySelector("#qb-form-container > div > form > div.quickbook-form_datePicker__gU5Ll > be-datepicker")
                        .shadowRoot.querySelector("#checkin-date").value = '{check_in_date}'
                """)
    sb.sleep(1)
    sb.execute_script(f"""
                    document.querySelector("#qb-form-container > div > form > div.quickbook-form_datePicker__gU5Ll > be-datepicker")
                        .shadowRoot.querySelector("#checkout-date").value = '{check_out_date}'
                """)
    sb.sleep(3)
    sb.execute_script("""
                    const checkbox = document.querySelector('be-checkbox[name="use-points"]');
                    if (checkbox) {
                        checkbox.checked = true;
                        checkbox.dispatchEvent(new Event('change', { bubbles: true }));
                    }
                """)
    sb.sleep(1)
    sb.click("button.be-button-shop")
    sb.sleep(6)
    card_info = 'div[data-booking-status="BOOKABLE"] [class*="HotelCard_info"]'
    hotels = sb.cdp.select_all(card_info)
    print("Hyatt Hotels in %s:" % location)
    print("(" + sb.cdp.get_text('span[class*="summary_destination"]') + ")")
    if len(hotels) == 0:
        print("No availability over the selected dates!")
    for hotel in hotels:
        info = hotel.text.strip()
        if "Avg/Night" in info and not info.startswith("Rates from"):
            name = info.split("  (")[0].split(" + ")[0].split(" Award Cat")[0]
            name = name.split(" Rates from :")[0]
            price = "?"
            if "Rates from : " in info:
                price = info.split("Rates from : ")[1].split(" Avg/Night")[0]
            print("* %s => %s" % (name, price))