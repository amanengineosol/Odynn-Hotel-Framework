...
class HyattScraper:
    ...
    def build_response(self, success, data, status_code, error=None):
        resp = {"success": success, "data": data, "status_code": status_code}
        if error:
            resp["error"] = error
        return resp
    ...
    async def _register_stealth_script(self, browser):
        """Injects pre-navigation stealth script for anti-bot evasion."""
        stealth_js = r"""
        (() => {
        ...
        registered = False
        try:
            # Preferred: browser-level CDP registration
            if hasattr(browser, "cdp") and hasattr(browser.cdp, "send"):
                await browser.cdp.send("Page.addScriptToEvaluateOnNewDocument", {"source": stealth_js})
                registered = True
                logger.info("Stealth script registered via browser.cdp")
            elif hasattr(browser, "main_tab") and hasattr(browser.main_tab, "cdp"):
                await browser.main_tab.cdp.send("Page.addScriptToEvaluateOnNewDocument", {"source": stealth_js})
                registered = True
                logger.info("Stealth script registered via browser.main_tab.cdp")
        except Exception as e:
            logger.warning(f"Failed to register stealth script pre-navigation: {e}")

        return stealth_js, registered

    async def _navigate_and_search(self, page):
        """Navigates, fills data, handles shadow DOM."""
        logger.info("Navigating Hyatt homepage and performing search...")

        await page.save_screenshot("hyatt.png")
        # Wait for search input to appear
        await page.wait_for('input[id="search-term"], input[data-id="location"]', timeout=30)
        loc_input = await page.query_selector('input[id="search-term"], input[data-id="location"]')
        await loc_input.click()
        await asyncio.sleep(1)
        await loc_input.send_keys(self.location)
        await asyncio.sleep(3)

        # Select first suggestion if appears
        suggestions = await page.query_selector_all('li[data-js="suggestion"], ul[role="listbox"] li')
        if suggestions:
            await suggestions[0].click()
            await asyncio.sleep(2)

        # Handle date input in shadow root
        logger.info("Injecting dates into Shadow DOM...")
        await page.evaluate(f"""
            (() => {{
                const dp = document.querySelector('be-datepicker');
                if (dp && dp.shadowRoot) {{
                    const ci = dp.shadowRoot.querySelector('#checkin-date');
                    const co = dp.shadowRoot.querySelector('#checkout-date');
                    if (ci) ci.value = '{self.check_in_date}';
                    if (co) co.value = '{self.check_out_date}';
                }}
            }})();
        """)
        ...
        await page.evaluate("""
            (() => {
                const cb = document.querySelector('be-checkbox[name="use-points"]');
        ...
        # Click Find Hotels
        logger.info("Clicking 'Find Hotels'...")
        await page.evaluate("""
            (() => {
                const btn = document.querySelector('button.be-button-shop');
                if (btn) btn.click();
            })();
        """)
        await asyncio.sleep(10)

        # Scroll bottom to load content
        await page.evaluate("window.scrollTo(0, document.body.scrollHeight);")
        await asyncio.sleep(5)
        return True

    async def _extract_html_and_parse(self, page):
        """Extracts the room cards data."""
        logger.info("Extracting room card data...")
        try:
            data = await page.evaluate("""
                (() => {
                    const cards = Array.from(document.querySelectorAll('.room-rate-card-wrapper'));
                    return cards.map(c => ({
                        room_type_code: c.getAttribute('data-room-type-code') || 'N/A',
                        title: c.querySelector('.room-title')?.innerText || '',
                        description: c.querySelector('.truncate-text.room_description')?.innerText || '',
                        image_url: c.querySelector('.room-card-carousel-wrapper img')?.src || '',
                        rate_type: c.querySelector('.room-rate-content.points-rate span.b-col-7')?.innerText || '',
                        point_value: c.querySelector('.room-rate-content.points-rate > span:last-child')?.innerText || ''
                    }));
                })();
            """)
            logger.info(f"Extracted {len(data)} rooms.")
            return data
        except Exception as e:
            logger.error(f"HTML extraction failed: {e}")
            return []

    async def get_search_data(self, hotel_id, check_in_date, check_out_date, guest_count=1):
        """Entry point."""
        parts = hotel_id.split("-", 1)
        ...
        try:
            stealth_js, registered = await self._register_stealth_script(browser)
            page = await browser.get(BASE_URL)
            await asyncio.sleep(3)

            # fallback stealth if not registered pre-navigation
            if not registered:
                await page.evaluate(stealth_js)

            # accept cookie / reject banners
            await page.evaluate("""
                (() => {
            ...
            self.structured_room_data = await self._extract_html_and_parse(page)

        except Exception as e:
            logger.exception(f"Fatal scraping error: {e}")
            return self.build_response(False, [], 500, str(e))
        finally:
            try:
                await browser.stop()
            except Exception:
        ...
        if self.structured_room_data:
            return self.build_response(True, self.structured_room_data, 200)
        else:
            return self.build_response(False, [], 204, "No room data found.")
...
if __name__ == "__main__":
    async def run():
        scraper = HyattScraper()
        result = await scraper.get_search_data(
            hotel_id="ancza-Hyatt Place Edmonton-West",
