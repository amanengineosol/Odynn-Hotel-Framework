...
# from scrapling.fetchers import Fetcher   # HTTP fetcher
# import json
#
# f = Fetcher()  # simple HTTP fetcher (faster than browser)
#
# params = {
#     # example params you observed in the browser request;
#     # adapt these to match the Hyatt querystring or payload from your logs
#     "spiritCode": "goazg",
#     "rooms": "1",
#     "adults": "1",
#     "location": "Hyatt Place Goa Candolim",
...
# }
#
# resp = f.get("https://www.hyatt.com/shop/service/rooms/roomrates/goazg", params=params, allow_redirects=True)
...
# except Exception:
#     print("Non-JSON response; saved raw HTML / text.")


from scrapling.fetchers import StealthyFetcher
import json, time

fetcher = StealthyFetcher(headless=True)   # headless True but Camoufox makes it look real

# Simple fetch: returns a Response-like object; Scraping engine will wait for JS by default
page = fetcher.fetch("https://www.hyatt.com/")           # initial page (dom + JS executed)
print("initial page status:", page.status)

# If you need to emulate clicking the 'Find Hotels' button, StealthyFetcher gives you two approaches:
# 1) If Hyatt exposes a GET/POST endpoint with query params (HyattSearch), craft that URL and fetch it
page2 = fetcher.fetch("https://www.hyatt.com/HyattSearch?locale=en-US&spiritCode=goazg&checkinDate=2026-01-07&checkoutDate=2026-01-08&rooms=1&adults=1")
print("search page status:", page2.status)

# 2) If you need to interact with the page (click), some Scrapling session APIs let you run JS
# (pseudo-code — the real Session API is documented in the docs):
# session = fetcher.open_session()
# page = session.fetch("https://www.hyatt.com/")
# session.eval("document.querySelector('button[data-js=\"quickbookSearchFormButton\"]').click()")
# wait and then check session.last_responses / session.fetch("...") results
params = {
    # example params you observed in the browser request;
    # adapt these to match the Hyatt querystring or payload from your logs
    "spiritCode": "goazg",
...
# If the room rates call was produced by an XHR, you can then fetch the roomrates API directly
api_resp = fetcher.fetch("https://www.hyatt.com/shop/service/rooms/roomrates/goazg?spiritCode=goazg&rooms=1&adults=1&location=Hyatt%20Place%20Goa%20Candolim&checkinDate=2026-01-07&checkoutDate=2026-01-08&kids=0&rate=Standard&suiteUpgrade=true")
print("roomrates status:", api_resp.status)
try:
    rr = api_resp.json()
